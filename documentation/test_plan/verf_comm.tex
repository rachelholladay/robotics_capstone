...
\textbf{Description:} A loss of connection occurs when a robot agent and the off-board processing unit are unable to send data between each other. As per \njnote{REF REQUIREMENT ABOUT PERFECT COMMUNICATION}, the robot system expects near perfect communication. This means that a failure resulting in intermittent or sparse connection will be treated equivalently to no connection. \\
