\documentclass[a4paper,10pt]{article}
%\documentclass{spec}
\usepackage[utf8]{inputenc}
\usepackage{color}
\usepackage[margin=1in]{geometry}
\usepackage{verbatim}
\usepackage{graphicx}
\usepackage{float}
\usepackage{ifthen,version}
\usepackage[usenames,dvipsnames]{xcolor}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{array,longtable}
\usepackage[nottoc]{tocbibind}
\newcounter{qcounter}
\newcounter{question}
\newcounter{row}

% Definition
\usepackage[english]{babel}
\usepackage{amsthm}
\theoremstyle{definition}
\newtheorem{definition}{Definition}[section]

% Create Pretty Priority Table
\makeatletter
\newtoks\@tabtoks
\newcommand\addtabtoks[1]{\@tabtoks\expandafter{\the\@tabtoks#1}}
\newcommand*\resettabtoks{\@tabtoks{}}
\newcommand*\printtabtoks{\the\@tabtoks}
\makeatother

\newcommand\CheckTable[1]{%
  \setcounter{question}{0}
  \setcounter{row}{0}
  \resettabtoks
  \loop\ifnum\therow<#1\relax
    \stepcounter{row}
    \addtabtoks{& $\square$ & $\square$ & $\square$ & $\square$ & $\square$ & $\square$ & $\square$ \\}%
  \repeat
  \begin{longtable}{>{\stepcounter{question}Priority}l*{7}{c}}
    \multicolumn{1}{c}{} & Lowest (1) & Low & Medium-Low & Neutral (4) & Medium-High & High & Highest (7) \\
    \printtabtoks
  \end{longtable}%
}

% Tracking Changes
\usepackage[final]{changes}
%\usepackage{changes}
%\definechangesauthor[name={Per cusse}, color=orange]{per}
\setremarkmarkup{(#2)}

% Labels in IEEE format
\newcommand{\sref}[1]{Sec. \ref{#1}} % Section
\newcommand{\figref}[1]{Fig.\ref{#1}} % Figure
\newcommand{\frref}[1]{FR\ref{#1}} % Functional Requirement

\usepackage{ifthen}
\usepackage[usenames,dvipsnames]{xcolor}
\newboolean{include-notes}
\setboolean{include-notes}{true}
\newcommand{\njnote}[1]{\ifthenelse{\boolean{include-notes}}%
 {\textcolor{yellow}{\textbf{NJ: #1}}}{}}
\newcommand{\rhnote}[1]{\ifthenelse{\boolean{include-notes}}%
 {\textcolor{red}{\textbf{RH: #1}}}{}}
\newcommand{\dznote}[1]{\ifthenelse{\boolean{include-notes}}%
 {\textcolor{green}{\textbf{DZ: #1}}}{}}
\newcommand{\yjnote}[1]{\ifthenelse{\boolean{include-notes}}
 {\textcolor{limegreen}{\textbf{YJ: #1}}}{}}

\begin{document}

\begin{titlepage}
	\centering
	{\scshape\LARGE Carnegie Mellon University \par}
	\vspace{1cm}
	{\scshape\Large Robotics Capstone Project\par}
	\vspace{1.5cm}
	{\huge\bfseries Concept Evaluation\par}
	\vspace{2cm}
	{\Large\itshape Friction Force Explorers:\par}
	\vspace{1cm}
	{\Large\itshape Don Zheng\par}
	{\Large\itshape Neil Jassal\par}
	{\Large\itshape Yichu Jin\par}
	{\Large\itshape Rachel Holladay\par}
  {\njnote{In class last week he mentioned a version number, we may want to consider putting it here}}
	\vfill
	supervised by\par
	Dr.~David \textsc{Wettergreen}

	\vfill

% Bottom of the page
	{\large Version 1\par
	\today\par}
\end{titlepage}

\tableofcontents
\listoffigures
\clearpage

%This is \added[remark={has to be in it}]{new} text.
%This is \deleted[remark=obsolete]{unnecessary}text.
%This is \replaced[remark={bad wording}]{nice}{bad} text.

\input{system_description.tex}
\input{concept_operation.tex}
\input{use_cases.tex}
\input{artistic_sketch.tex}
\input{subsystem_descriptions.tex}
\input{trade_studies.tex}
\input{software_architecture.tex}
\input{install.tex}

%\pagebreak
%\listofchanges
%\pagebreak

%{\footnotesize
%\bibliographystyle{ieeetr}
%\bibliography{references}}

\end{document}