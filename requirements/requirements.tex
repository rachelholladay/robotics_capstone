...
\rhnote{Break this up into 1 function (omnidirectional) and 2 nonfunctional (position and rotational accuracy)} \\
...
\rhnote{I dont understand the feedback on this section} \\
...
\rhnote{10 seconds is nonfunctional requirement. So is safely and accurately.}
\begin{functional_requirement}{Turn on or off writing tool}{1}
\label{fr:on_tool}
\item All robot agents need to be able to enable or disable use of the writing implement. The robot must also have the ability to move safely and accurately regardless of the state of the writing implement. Not all drawings are contiguous lines, and as such the robots must be able to disable the writing tool to move to a new drawing location. \added[remark={NJ, V2}]{Individual robots must be able to enable or disable the writing implement within 10 seconds.}
\end{functional_requirement}
...
\rhnote{Add functional requirement on battery power.}
...
\rhnote{One requirement on weight. One on size. on on drop height.}
...
\rhnote{From DW: Lots of variability in human performance. Maybe use rate of
drawing implement consumption or \% of duty cycle drawing}
\begin{nonfunctional_requirement}{Efficiency}{2}
\item Robot system must be efficient and complete drawing tasks quickly. Timeliness can be measured by the amount of time taken to complete drawings. Robot planning must also exist in such a manner that evenly splits the workload among all robot agents working in the system. Therefore, a noticeable speedup is expected when using two robots as opposed to one. \added[remark={NJ, V2}]{The system should perform as fast or faster than a human attempting the same drawing task.}
\end{nonfunctional_requirement}
